...
class Planet: HeavenlyBody {
    ...
    let revolutionSpeed = 5 // 公転スピード（共通）
    ...
    func makeRotatingPlanet(diffuse: UIImage, radius: CGFloat, distance: Float) -> SCNNode {
        let mercury = makePlanet(geometry: SCNSphere(radius: radius), diffuse: diffuse, specular: nil, emission: nil, normal: nil, position: SCNVector3(distance, 0, 0))
